...
if [ ! $# -ne 1 ]; then
    if [ "commit" = $1 ]; then
        docker commit development-container-for-ros-2-humble-on-m1-2-mac_for_${USER}_container-asv_loyola development-container-for-ros-2-humble-on-m1-2-mac_for_${USER}:latest
...
if [ ! $# -ne 1 ]; then
    if [ "delete" = $1 ]; then
        ...
        docker stop $CONTAINER_ID
        docker rm $CONTAINER_ID -f
        docker image rm development-container-for-ros-2-humble-on-m1-2-mac_for_${USER}-asv_loyola
        exit 0
    fi
fi

DOCKER_OPT=""
DOCKER_NAME="development-container-for-ros-2-humble-on-m1-2-mac_for_${USER}_container-asv_loyola"
    ...
    if [ ! $# -ne 1 ]; then
        if [ "xrdp" = $1 ]; then
            ...
            docker run ${DOCKER_OPT} \
                ...
                development-container-for-ros-2-humble-on-m1-2-mac_for_${USER}-asv_loyola:latest
        else
            docker run ${DOCKER_OPT} \
                --name=${DOCKER_NAME} \
